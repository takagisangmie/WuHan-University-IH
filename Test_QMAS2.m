function Test_QMAS2(cover_dir,stego_dir,payload,cover_QF,attack_QF)
% 424  384
...
cover_num = 1000; %number of test images
anti_jpeg_cover_dir = [cover_dir,'_anti_jpeg_qf=80'];  if ~exist(anti_jpeg_cover_dir,'dir'); mkdir(anti_jpeg_cover_dir); end
...
for i_img = 1:cover_num
    ...
    anti_jpeg_cover_path=fullfile([anti_jpeg_cover_dir,'\',num2str(num),'.jpg']);
    %% stegaongraphy
    stego_step = ones(8,8); % 
    % pre-process
    try
    ...
    if bit_error_number==0
        fprintf('img_%d_bit_error_number is 0\n',i_img);
        
        imwrite(imread(cover_Path),anti_jpeg_cover_path,'quality',cover_QF);
        num=num+1;
    end
